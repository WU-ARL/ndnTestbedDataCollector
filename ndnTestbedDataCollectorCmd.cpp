...
public:
  ...
  onData(const ndn::Interest& interest, ndn::Data& data, std::string linkPrefix)
  {
    ...
    if(DEBUG) std::cout << "onData(): before cmpName.isPrefixOf()" << std::endl;
    if (cmpName.isPrefixOf(interest.getName()) )
    {
      if(DEBUG) std::cout << "onData(): if cmpName.isPrefixOf()" << std::endl;
    ...
    {  
      if(DEBUG) std::cout << "onData(): else if cmpName.isPrefixOf()" << std::endl;
      reply.wireDecode(data.getContent().blockFromValue()); 
    ...
    if(DEBUG) std::cout << "onData(): bottom" << std::endl;
    m_io.stop();
  } 
...
*/
    ...
    if (!singleScript)
    {
      if(DEBUG) std::cout << "!singleScript: schedule next fetch" << std::endl;
      m_scheduler.scheduleEvent(time::seconds(m_pollPeriod), bind(&NdnMapServer::sendInterests, this));
    } else {
      if(DEBUG) std::cout << "singleScript: processSingleEvent()" << std::endl;
      processSingleEvent();
      if(DEBUG) std::cout << "singleScript: returned from processSingleEvent(), callng exit(0)" << std::endl;
      exit(0);
    }
  }
